...
model = dict(
    ...
    criteria=[
        dict(
            ...
            loss_weight=0.65,
            ignore_index=-1,
            # 类别占比 0.35, 0.47, 0.09, 0.07 → 反比权重，归一化后最小为 1.0
            weight=[1.34, 1.0, 9.22, 6.71]
        ),
        dict(
            type='LovaszLoss',
            mode='multiclass',
...
# 必须指向本机存在的、S3DIS 预处理后的目录（结构: data_root/Area_1/房间名/coord.npy, segment.npy, normal.npy）
# 原路径为 AutoDL 机器路径，本地需改为实际路径，否则 num_samples=0 报错
data_root = "/home/poker/workspace/11_third_projs/data/train_data/00_oldversion/by3_class4_v1/train"
...
data = dict(
    ...
    train=dict(
        ...
        transform=[
            ...
            dict(type="SphereCrop", sample_rate=0.55, mode="random"),
            dict(type="SphereCrop", point_max=960000, mode="random"),
            dict(type="ToTensor"),
            dict(
                type="Collect",
